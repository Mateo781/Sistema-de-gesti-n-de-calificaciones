...
//  PROCESAR GUARDADO DE EVALUACIÓN Y NOTAS
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_evaluacion'])) {
    ...
    if (empty($titulo) || $id_tipo === 0 || $id_periodo === 0 || empty($fecha) || empty($fecha_fin) || $id_cmd === 0) {
        $mensaje_err = "Faltan completar campos mandatorios de la evaluación (incluyendo la fecha de finalización).";
    } else {
        try {
            ...
            // A. Insertar la evaluación principal
            $stmtEv = $pdo->prepare("
                INSERT INTO evaluaciones (id_curso_materia, id_tipo_evaluacion, id_periodo, titulo, descripcion, archivo_pdf, fecha_evaluacion, fecha_fin)
                VALUES (:cmd, :tipo, :periodo, :titulo, :descr, :pdf, :fecha, :fecha_fin)
            ");
            $stmtEv->execute([
                ':cmd'         => $id_cmd,
                ':tipo'        => $id_tipo,
            ...
            $stmtAud->execute([
                ...
                ':val' => json_encode(["titulo" => $titulo, "fecha" => $fecha, "fecha_fin" => $fecha_fin, "id_curso_materia" => $id_cmd, "archivo" => $ruta_pdf_final]), 
            ...
            <form method="POST" action="index.php?p=prof_evaluaciones&id_curso=<?= $curso_sel ?>&id_materia=<?= $materia_sel ?>" enctype="multipart/form-data">
                ...
                <div class="eval-card">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group-custom">
                            <label>Fecha de Fin / Límite:</label>
